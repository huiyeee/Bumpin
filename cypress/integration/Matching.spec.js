...
describe('Enter the hallway', ()=> {
    ...
    it('Click enter the game room button', () => {
        ...
        cy.get('[data-cy=main-text]').should('contain', `Matching...`);
    })

    it('Checks if user can keep walking', () => {
        cy.get('[data-cy=keep-walking-button]').should('contain','Keep');
        cy.get('[data-cy=keep-walking-button]').click();
        cy.get('[data-cy=keep-walking-button]').should('contain','Keep');
    })
